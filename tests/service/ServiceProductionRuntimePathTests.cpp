    ...
    const std::chrono::milliseconds deadline = 30min)
{
    ...
    result.production_runtime_script_provider = std::move(provider);
    result.runtime_script_backend.task_deadline = deadline;
    return result;
}
...
{
    {
        ...
        if (opened) {
            ...
            check(client.response().has_value(),
                  "service ownership acknowledgement is still delivered");
            check(wait_until([&] {
                      const auto value =
                          opened.application->runtime_task_snapshot("fail");
                      return value && !value->running;
                  }),
                  "provider failure publishes terminal state");
            const auto value =
                opened.application->runtime_task_snapshot("fail");
            check(value && value->exit_code
                      == service_runtime::runtime_script_task_failure_exit_code,
                  "null provider publication fails closed in the real backend");
            opened.application->stop();
        }
    }
    {
        ...
        auto opened = app::ServiceApplication::open(
            project.options(), dependencies(provider, 25ms));
...
void test_duplicate_config_cancel_and_concurrent_shutdown()
{
    ...
    check(static_cast<bool>(
              client.submit("solve", 6, "same", R"({"task":"one"})")),
    ...
    check(static_cast<bool>(client.submit("stop_scheduler", 7, "same")),
    ...
    check(static_cast<bool>(
              client.submit("solve", 10, "application-stop",
                            R"({"task":"one"})")),
          "application shutdown fixture submits");
    check(client.response().has_value(),
          "application shutdown fixture is acknowledged");
    check(wait_until([&] { return provider->pins.load() == 3; }),
